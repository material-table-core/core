...
export default function MTableBodyRow(props) {
  ...
  if (props.options.selection) {
    renderedColumns.splice(0, 0, renderSelectionColumn());
  }
  if (
    props.actions &&
    ...
    if (props.options.actionsColumnIndex === -1) {
      renderedColumns.push(renderActions());
  ...
  if (props.detailPanel) {
    if (props.options.detailPanelColumnAlignment === 'right') {
      renderedColumns.push(renderDetailPanelColumn());
    } else {
      renderedColumns.splice(0, 0, renderDetailPanelColumn());
    }
  }

  // Lastly we add tree data icon
  if (props.isTreeData) {
    renderedColumns.splice(0, 0, renderTreeDataColumn());
  }
