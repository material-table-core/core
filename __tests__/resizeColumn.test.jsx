...
vi.mock('@utils/common-values', async (importOriginal) => {
  const mod = await importOriginal();
  return {
    ...mod,
    reducePercentsInCalc: (calc, scrollWidth) =>
      mod.reducePercentsInCalc(simplifyCalc(calc), windowInnerWidth)
  };
});
...
describe('Resize Column', () => {
  ...
  const windowGetComputedStyle = (el) => {
    ...
    if (!width || !width.startsWith('calc(')) return style;
    const evaluated = `${evalCalc(width)}px`;
    return new Proxy(style, {
      get(target, prop) {
        if (prop === 'width') return evaluated;
